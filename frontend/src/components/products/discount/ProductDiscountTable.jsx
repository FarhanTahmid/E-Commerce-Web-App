...
const ProductDiscountTable = () => {
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/product-discounts/fetch-product-discount/`, {
                headers: {
                    Authorization: `Bearer ${Cookies.get("accessToken")}`,
                    "Content-Type": "application/json"
                }
            });
            setCategories(response.data.product_discount);
        } catch (error) {
    ...
    const filteredCategories = categories.filter(category =>
        category.discount_name.toLowerCase().includes(searchTerm.toLowerCase())
    );
                                            ...
                                            <tbody>
                                                {filteredCategories.length === 0 ? (
                                                    ...
                                                    <tr key={category.id} className='single-item chat-single-item'>
                                                        ...
                                                        <td className="truncate-text">{category.discount_amount}</td>
                                                    </tr>
                                                )))}
                                            </tbody>
                                        </table>
