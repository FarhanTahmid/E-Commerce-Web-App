...
const ProductBrandTable = () => {
    ...
    const API_BASE_URL = 'http://127.0.0.1:8000/server_api/product/product-brand';
    ...
    const fetchCategories = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/fetch-product-brands/`, {
                headers: {
                                        ...
                                        <table className="table table-hover dataTable no-footer" id='projectList'>
                                            ...
                                            <tbody>
                                                {filteredBrands.length === 0 ? (
                                                    ...
                                                    <tr key={brand.id} className='single-item chat-single-item'>
                                                        <td>{index + 1 + page * rowsPerPage}</td> {/* Serial Number */}
                                                        <td className="truncate-text">
                                                            <Link to={`/products/brand/${brand.id}`} className='fw-bold'>
                                                                {brand.brand_name}
                                                            </Link>
                                                        </td>
